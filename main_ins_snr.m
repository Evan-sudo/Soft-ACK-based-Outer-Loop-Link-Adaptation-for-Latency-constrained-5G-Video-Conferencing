...
pdsch.PRBSet = 0:ceil((carrier.NSizeGrid-1)*u_d_ratio);     % Full band allocation
...
tx_tb = [tx;tx;tx;tx;tx;tx;tx;tx;tx;tx;tx;tx;zeros(50000*1500,1)];
...
%        zlabel("Magnitude");
        ...
        Pr = 1/2 * ((erf((snr_pr-SINR_REF(cqi_pr)+a)/sqrt(2))+1));  
        if soft_ack_true
            if ~blkerr
                 if Pr < 0.5      % set the threshold
                     snr_off = max(-max_dev-4,snr_off-9*blkerr*delta);    % receive low-margin-ack
                     disp('Soft ACK!')
                 else 
                     snr_off = min(snr_off+(1-blkerr)*delta,max_dev);    % receive high-margin-ack
                 end
            else 
                snr_off = max(-max_dev-4,min(snr_off+(1-blkerr)*delta-9*blkerr*delta,max_dev));
            end
...
%         if ~mod(length(log_),15)
%            cqi_t = find(diff == min(diff));  % mcs for the next tb
%         end
        cqi_t = find(diff == min(diff));  % mcs for the next tb
        ...
        disp("Slot "+(nSlot)+". "+statusReport+ " CQI "+(cqi_t)+" SNR "+(snr_avg)+" dB"+" SNR offset "+(snr_off)+"Doppler shift: "+(channel.MaximumDopplerShift));
        
        log_ = [log_; nSlot];
        mod_flag = ~ mod(length(log_),20);
...
%         end
% 
%         if ~ mod(length(log_),BW_expand*50)
...
%         end
        ...
        if length(log_)>500
            if ~mod(length(log_),1000)
                b = length(bler(bler== 1))/length(bler);
                bler_acc = [bler_acc,b];
            end
        end
