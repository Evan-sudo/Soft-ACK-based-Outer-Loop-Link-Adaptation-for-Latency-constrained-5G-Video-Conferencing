...
%        zlabel("Magnitude");
        ...
        if soft_ack_true
            if ~blkerr
                 if Pr < 0.8      % set the threshold
                     snr_off = max(-max_dev,snr_off-9*blkerr*delta);    % receive low-margin-ack
                     disp('Soft ACK!')
                 else 
                     snr_off = min(snr_off+(1-blkerr)*delta,max_dev);    % receive high-margin-ack
                 end
            else 
                snr_off = max(-max_dev,min(snr_off+(1-blkerr)*delta-9*blkerr*delta,max_dev));
            end
...
%         end
        cqi_t = find(diff == min(diff));  % mcs for the next tb
        log_snr_olla = [log_snr_olla, snr_avg];  % log snr for conventional olla
...
bler_snr = [bler_snr, bler_all];  % record snr for different snrs
end
